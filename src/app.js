...
const HELP = [
  ...
  ['Карточки карусели',
   ...
   'Если просто вставить большой кусок текста без меток //, кнопка ' +
   '«Разбить автоматически» над полем сама расставит метки — режет по ' +
   'абзацам (пустая строка между ними) так, чтобы текст помещался на ' +
  ...
  ['Переполнение текста',
   ...
   'Уменьши кегль или межстрочный интервал, перенеси часть текста на ' +
   'другую карточку — или воспользуйся «Разбить автоматически».'],
  ['Форматирование',
...
/*
 * Разбивает вставленный текст на карточки по вместимости: пустая строка —
 * граница абзаца, каждый абзац становится одной строкой карточки. Абзацы
 * набираются в карточку, пока блок текста (тем же buildBlock/измерением,
 * что и сам рендер) помещается в белую карточку без фото; как только новый
 * абзац переполнил бы её — начинается следующая карточка. Существующие
 * метки //N в тексте не сохраняются — считается, что функция запускается
 * на только что вставленном необработанном тексте, а не на готовой
 * раскладке (иначе она бы её просто перемешала).
 */
function autoSplitText() {
  if (!state.fontsReady) { say('Шрифты ещё грузятся — попробуй через секунду'); return; }
  ...
  if (!confirm('Текущая раскладка на карточки будет заменена — метки //N расставятся заново по объёму текста. Продолжить?')) return;
  ...
  const withGaps = paras => paras.flatMap((p, i) => i ? ['', p] : [p]);

  const cardParas = [[]];
  for (const para of paragraphs) {
    const last = cardParas[cardParas.length - 1];
    const attempt = last.concat(para);
    const { total } = buildBlock(ctx, { kind: 'card', lines: withGaps(attempt) }, L, style, maxWidth);
    // last.length проверяем, чтобы не зациклиться: одинокий абзац, который
    // сам по себе больше budget, всё равно остаётся на своей карточке —
    // её потом подсветит индикатор переполнения (см. renderCard)
    if (total > budget && last.length) cardParas.push([para]);
    else cardParas[cardParas.length - 1] = attempt;
  }

  state.cardsText = cardParas
    .map((paras, i) => '//' + (i + 1) + '\n' + withGaps(paras).join('\n'))
    .join('\n\n');
  setEditorValue(state.cardsText, false);
  syncCards(); buildPreviews(); saveProject();
  say('Разбито на карточек: ' + cardParas.length);
}
