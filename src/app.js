...
const HELP = [
  ...
  ['Карточки карусели',
   ...
   'в одну и без разрезания абзаца на части. Заменяет текущую раскладку, ' +
 ...
 * на несколько. Строки внутри абзаца (например, короткий заголовок на своей
 * строке и текст под ним) остаются как есть — это то же самое, что «строка
 * целиком жирная — подзаголовок» из обычной разметки. Длинный абзац может
 * не поместиться на карточку целиком — тогда сработает индикатор
 * переполнения (см. renderCard), и его можно будет разбить вручную.
